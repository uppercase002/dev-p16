    ...
    if ma_session is not None:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if matric_Mem != matricule and tel_Mem != tel:
                    if Etudiant.objects.filter(Matricule = matricule).exists():
                        messages.error(request,"Ce matricule existe déjà !")
                    elif Etudiant.objects.filter(telephone = tel).exists():
                        messages.error(request,"Ce tel existe déjà !")
    ...
    if ma_session is not None:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                elif mat_Mem != matric and tel_Mem != tel and mail_Mem != mail:
...
def matiere(request):
    ...
    if ma_session is not None:
        ...
        if request.method == 'POST':
            ...
            if Matiere.objects.filter(Libelle = mat.lower()).exists():
                if Matiere.objects.get(Libelle = mat.lower()).Departement == depart:
